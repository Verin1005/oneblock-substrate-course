...
fn create_claim_failed_when_claim_already_exists() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(PoeModule::create_claim(Origin::signed(1),claim), Error::<Test>::ClaimAlreadyExists);
    });
}
...
fn revoke_claim_works() {
    new_test_ext().execute_with(|| {
        ...
        // 已被撤销
        let bounded_claim = BoundedVec::<u8, <Test as Config>::MaxClaimLength>::try_from(claim).unwrap();
...
fn revoke_claim_failed() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(PoeModule::create_claim(Origin::signed(1),claim.clone()));
        // 撤销一个不存在的存证
        assert_noop!(PoeModule::revoke_claim(Origin::signed(1), vec![0, 0]), Error::<Test>::ClaimNotExists);
        // 不是存证的拥有者
...
fn transfer_claim_failed() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(PoeModule::create_claim(Origin::signed(1),claim.clone()));
        // 是否转给了自己
        assert_noop!(PoeModule::transfer_claim(Origin::signed(1), claim.clone(),1), Error::<Test>::TransferToSelf);
        // 不存在的存证
        assert_noop!(PoeModule::transfer_claim(Origin::signed(1), vec![0, 0],2), Error::<Test>::ClaimNotExists);
        // 不是存证的拥有者
        assert_noop!(PoeModule::transfer_claim(Origin::signed(2), claim.clone(),1), Error::<Test>::NotClaimOwner);
    });
}
